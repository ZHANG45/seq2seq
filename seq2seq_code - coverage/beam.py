...
class Beam(object):
    def __init__(self, size, vocab, n_best=1, cuda=True, length_norm=False, minimum_length=0):
        ...
        self.nextYs = [self.tt.LongTensor(size)
                       .fill_(vocab.stoi['<eos>'])]
        self.nextYs[0][0] = vocab.stoi['<sos>']

        # Has EOS topped the beam yet.
        self._eos = vocab.stoi['<eos>']
        self.eosTop = False
    ...
    def advance(self, wordLk, attnOut):
        ...
        * `attnOut`- attention at the last step
        ...
        if self.nextYs[-1][0] == self.vocab.stoi['<eos>']:
            self.allScores.append(self.scores)
            self.eosTop = True
