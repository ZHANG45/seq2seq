...
from generators import Generator, Generator_q
from attention import Attention
from DefinedParameters import  USE_COVERAGE, MAX_FERTILITY, USE_PRETRAINED_EMBEDDING
...
class Seq2Seq(nn.Module):
    def __init__(self,
        ...
        self.generator_q = Generator_q(src_vocab_size, hidden_size, dropout_p)
...
class Encoder(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size, dropout_p, emb_vectors):
        ...
        if USE_PRETRAINED_EMBEDDING:
            self.embed.weight.data.copy_(emb_vectors)
...
class Decoder(nn.Module):
    ...
    def forward(self, trg, encoder_states, last_states, fertility, src_mask):
        ...
        attn_weights = torch.zeros(encoder_states.size()[1],encoder_states.size(0),1).cuda() #BXlen(input)xlen(target)
